            ...
            <div className="flex items-center gap-2">
              ...
              {(overview.country || statsObj.country || playerData.country) && (
                <img
                  ...
                  alt={overview.country || statsObj.country || playerData.country}
          ...
          <tbody className="text-white">
            {matches.map((match, i) => (
              <tr key={i} className="border-b border-[#2C2C2C]">
                <td className="py-2">{match.date || match.created_at ? new Date(match.created_at || match.date).toLocaleDateString() : ""}</td>
                <td>{match.map}</td>
                <td className={`${
                                match.w === 1 || match.w === "1"
                                ? "text-green-400"
                                ...
                                ? "text-red-400"
                                : "text-yellow-400"
                              }`}>{match.w === 1 || match.w === "1" ? "Win" : match.w === 0 || match.w === "0" ? "Loss" : "?"}</td>
                <td>{`${match.k}/${match.d}`}</td>
                <td>{`${match.elod} ${match.elo}`}</td>
                <td><button className="group bg-[#2C2C2C] hover:bg-[#3a3a3a] text-white rounded flex items-center transition">
